...
def is_good_weather(owm_json) -> bool:
    """
    Простое правило:
    - clear, few clouds, scattered clouds -> хорошая
    - всё остальное (дождь, снег, грозы, туман, сильная облачность) -> плохая
    """
    try:
    ...
    if main == "clear":
        return True
    if main == "clouds" and wid in (800, 801, 802):  # мало/рассеянные облака
        return True

    # Всё остальное считаем плохой погодой
    return False
...
class PetWindow:
    def __init__(self):
        ...
        self.weather_json = None  # сюда кладём JSON от OpenWeather
    ...
    def _weather_tick(self):
        ...
        # Если нет активного действия (кормёжка, игра, сон) и питомец не перетаскивается
        if self.action_lock_ms == 0 and self.state != MoveState.DRAG:
            if self.weather_json is not None:
                ...
                # Задай имена гифок под свои файлы
                if good:
                    self._safe_loadanimation("happy")
                else:
                    self._safe_loadanimation("sad")
